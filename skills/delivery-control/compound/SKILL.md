...
- **Read before writing.** Rehydrate from these sources in order before extracting anything:
  ...
  2. `docs/live/contract.md` — approved slice contract, success criteria, and forbidden changes, when present
  3. `docs/live/progress.md` — what actually happened, what was verified, what failed?
  4. `docs/live/qa.md` — evaluator verdicts and defect evidence, when present
  5. `docs/live/runtime.md` — execution mode and baton history, when present
  6. `docs/reference/lessons.md` — existing lessons to avoid duplication
  7. `docs/reference/memory.md` — existing durable truths to avoid duplication
  8. Session history or conversation context — raw material for extraction
...
- **Respect slice-vs-durable scope.** If `docs/live/contract.md` exists, use it to understand the approved slice, but archive only what became a reusable lesson or durable project truth.
...
- Live docs (`docs/live/current-focus.md`, `docs/live/contract.md` when present, `docs/live/progress.md`, `docs/live/qa.md`, `docs/live/runtime.md`) with current-session state

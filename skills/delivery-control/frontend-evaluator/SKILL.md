...
- builder-side browser QA during implementation — use the implementation skill or builder workflow that owns the change
...
- Use the bundled [**Interactive Browser QA**](references/interactive-browser-qa.md) workflow as the canonical browser evaluation method. Follow it in full; do not replace it with a shorter private checklist.
- Treat `docs/live/contract.md` when present and `docs/live/qa.md` always as the canonical contract/evidence pair for the current slice.
...
- the bundled Interactive Browser QA workflow was completed in full
- every required user-visible claim was independently verified against the slice contract when present, with evidence tied to the observed state where it matters
...
Write the evaluation to `docs/live/qa.md` in markdown. When `docs/live/contract.md` exists, make the evidence matrix trace back to that slice contract.
...
1. Read `docs/live/contract.md` when present, then reconstruct any remaining acceptance scope from the user request, planner contract, and delivered browser-facing behavior.
2. Read and apply the bundled [**Interactive Browser QA**](references/interactive-browser-qa.md) methodology.
...
- summarizing the bundled QA methodology instead of actually running it

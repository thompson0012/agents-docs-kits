...
### Prose-Only Root Router
- Pattern: Root `AGENTS.md` carries routing intent only as prose, with no machine-readable manifest or validator to keep pointers and sections honest.
- Detection: startup guidance, local guides, and skill pointers look reasonable in markdown but drift from the filesystem or from each other without any failing check.
- Better default: keep root `AGENTS.md` lean, add a routing manifest, and validate the contract with executable tests and a validator script.

### Repo vs Template Truth Bleed
- Pattern: Agents treat root repo live docs and template live-doc scaffolds as interchangeable because both trees expose similar filenames.
- Detection: template live docs receive active progress/objective updates, or repo work cites `templates/base/docs/live/*` as current state.
- Better default: fence repo-vs-template scope explicitly at the root router and in the leaf guides, then test the distinction with negative controls.
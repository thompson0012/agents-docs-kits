...
Pressure-test the expanded router suite (`website-building`, `using-legal`, `using-sales`, `using-marketing`, `using-reasoning`, `using-research`, and `using-finance`) and decide whether the router model is mature enough before creating any additional family routers such as the deferred document-format family.
...
- Treat the document-format family (`docx`, `pdf`, `pptx`, `xlsx`) as a deferred candidate until the expanded router suite is pressure-tested.

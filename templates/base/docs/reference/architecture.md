...
- Inside scope: the template harness under `templates/base/`, including live docs that carry the active objective, runtime control state, continuity, and QA evidence for non-trivial work.
...
- Boundary note: `docs/live/runtime.md` and `docs/live/qa.md` extend the live-doc surface when explicit delivery control or independent evaluation is needed; they are not required for every trivial task.

...
**Color restraint philosophy:** See `.agents/skills-optional/using-design/design-foundations/SKILL.md` for the full rationale. In brief: 1 accent + neutrals for most pages. Remaining palette colors (orange, gold, blue, purple, notification) are reserved for data visualization only.
...
The Nexus palette is a neutral, warm-beige/teal system designed as a safe fallback — not the default for every site. For the format-agnostic hex palette and full rationale, see `.agents/skills-optional/using-design/design-foundations/SKILL.md`. Below is the CSS variable implementation.

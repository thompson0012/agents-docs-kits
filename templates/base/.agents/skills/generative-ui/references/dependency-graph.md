...
| --- | --- | --- | --- |
| `generating-design-tokens -> generative-ui` | prerequisite producer | No trustworthy design token spec exists yet | The hard dependency is the token artifact. If `docs/reference/design.md` or an equivalent token spec already exists, use that instead of regenerating it. |
| `design-foundations -> generative-ui` | fallback reference | Disposable prototype with no project-specific token spec | Use only when the work is exploratory and you label the fallback honestly. |
...
- Existing token docs satisfy the prerequisite; `generating-design-tokens` is the producer when that artifact is missing.

...
**Design guidance:** For styled spreadsheets (dashboard reports, branded workbooks), see `.agents/skills-optional/using-design/design-foundations/SKILL.md` for the default accent color and chart colors. Reserve color for emphasis — most cells should use default black text on white. Use the accent color sparingly (header rows, key totals). Financial model color conventions below are industry-standard overrides and take priority.

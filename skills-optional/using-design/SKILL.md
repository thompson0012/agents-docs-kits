...
Use this router when the task is primarily about design direction, visual-system definition, or design-specific browser UI work and more than one design child could fit. For reusable token-spec or brand-system work, route directly to the top-level `design-token-spec` skill instead.
...
| Reusable token spec or brand-system artifact | `load_skill("design-token-spec")` | Design token specs, brand systems, design specs, token architecture docs |
...
- `Route to design-token-spec (top-level skill).`
...
3. Token-spec and brand-system work belongs to the top-level `design-token-spec` skill, not this family.

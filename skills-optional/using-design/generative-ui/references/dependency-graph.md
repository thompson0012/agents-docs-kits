...
| --- | --- | --- | --- |
| `design-token-spec -> using-design/generative-ui` | prerequisite producer | No trustworthy design token spec exists yet | The hard dependency is the token artifact. `design-token-spec` is a top-level skill. If `docs/reference/design.md` or an equivalent token spec already exists, use that instead of regenerating it. |
...
- Existing token docs satisfy the prerequisite; the top-level `design-token-spec` skill is the producer when that artifact is missing.

...
Return a concrete ownership decision in this shape:
- `Update <surface>.`
- `Also update <paired-surface>.`
- `Do not update <surface>.`

The skill should end with a small cutover set, not a general explanation.

## Common Scenarios

| Scenario | Update | Do not update |
| --- | --- | --- |
| New durable subtree with local rules | new local `AGENTS.md`, nearest parent guide, and root/codemap if startup or high-value entrypoint changed | `docs/live/*` unless session state also changed |
| Durable policy changed but boundary rules did not | relevant `docs/reference/*` file | `AGENTS.md` unless the boundary contract also changed |
| Progress, blockers, verification, or next action changed | `docs/live/*` | `AGENTS.md`, `docs/reference/*`, and skills |
| Repo-local skill path or router metadata changed | owning skill docs plus root `AGENTS.md`, `.agents/router-manifest.json`, and `docs/reference/codemap.md` | unrelated live docs |
| Durable policy and repeatable procedure changed together | relevant `docs/reference/*` file and the owning skill package | updating only one of them |
...
## Workflow
1. Read the root `AGENTS.md`, then the nearest existing local `AGENTS.md` files for the subtree you are touching. Use `.agents/router-manifest.json` if the owning repo surface is unclear.
2. Decide whether the change affects boundary rules, durable truth, session state, or repeatable procedure.
...
- updating root routing when only the nearest parent inventory changed
- skipping `.agents/router-manifest.json` after a repo-local skill or entrypoint move
